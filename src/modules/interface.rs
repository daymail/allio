    ...
    fn event_handler(&mut self, key: KeyEvent) {
        ...
        match self.mode {
            InterfaceMode::Normal => match key.code {
                KeyCode::Char(':') | KeyCode::Char('i') => self.mode = InterfaceMode::Insert,
...
mod minor {
    ...
    pub fn render(
        ...
        state: &mut ListState,
        items: &[ModuleItem],
        visible_map: &[VisibleItem],
        mode: &InterfaceMode,
    ) {
        let mut list_items = Vec::new();
        let (color, current_mode) = match mode {
            InterfaceMode::Insert => (PALETTE.primary, "[I]"),
            InterfaceMode::Normal => (PALETTE.secondary, "[N]"),
        };
